...
function renderInstance(deck, node) {
  ...
  // Contains Figma-computed glyph layouts for overridden text.
  const dsdMap = new Map();
  for (const entry of node.derivedSymbolData ?? []) {
    const g = entry.guidPath?.guids?.[0];
    if (g) dsdMap.set(`${g.sessionID}:${g.localID}`, entry);
  }
  ...
  for (const ov of overrides) {
    ...
    // Try direct node lookup first, then overrideKey lookup
    const target = deck.getNode(targetId) ?? okMap.get(targetId);
    if (!target) continue;

    // Text override — replace characters AND apply the Figma-computed glyph
    // layout from derivedSymbolData so glyph positions match the new text.
    if (ov.textData?.characters != null && target.textData) {
      ...
      // Apply matching derivedTextData from derivedSymbolData
      const dsd = dsdMap.get(targetId);
      if (dsd?.derivedTextData) {
        const origDerived = target.derivedTextData;
        restores.push(() => { target.derivedTextData = origDerived; });
  ...
  const rx = Math.min(symbol.cornerRadius ?? 0, symSize.w / 2, symSize.h / 2);
  let { defs, bg } = renderRoundedRectFillStack(deck, getFillPaints(symbol), symSize.w, symSize.h, rx);
  ...
  let content = [defs, bg, inner].filter(Boolean).join('\n');
  if (!content) return '';
  if (needsScale) {
